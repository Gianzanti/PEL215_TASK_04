...
class Map:
    def __init__(self, xsize, ysize, grid_size):
        ...
        self.grid_position_m = np.array(
            [
                np.tile(
                    np.arange(0, self.xsize * self.grid_size, self.grid_size)[:, None],
                    (1, self.ysize),
                ),
                np.tile(
                    np.arange(0, self.ysize * self.grid_size, self.grid_size)[
                        :, None
                    ].T,
                    (self.xsize, 1),
                ),
    ...
    def update_map(self, pose, z):
        ...
        theta_to_grid = (
            np.arctan2(dx[1, :, :], dx[0, :, :]) - pose[2]
        )  # matrix of all bearings from robot to cell

        # Wrap to +pi / - pi
        theta_to_grid[theta_to_grid > np.pi] -= 2.0 * np.pi
        theta_to_grid[theta_to_grid < -np.pi] += 2.0 * np.pi

        dist_to_grid = scipy.linalg.norm(
            dx, axis=0
        )  # matrix of L2 distance to all cells from robot
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(state.T))):
        ...
        plt.imshow(1.0 - 1.0 / (1.0 + np.exp(map.log_prob_map)), "Greys")
        plt.pause(0.005)
    ...
    plt.clf()
    plt.imshow(
        1.0 - 1.0 / (1.0 + np.exp(map.log_prob_map)), "Greys"
    )  # This is probability
